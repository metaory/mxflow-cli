...
global.runCommands = async(commands = [], data = {}) => {
  for (const cmdName of commands) {
    ...
    const parsedCommand = pupa(cmd || '', data)

    log.info(cmdName + ':', parsedCommand)

    await sleep(cfg.sleep_between_commands) // XXX for no reason!

    const [, pickOff] = cmdName.split('go_pick_')
    if (pickOff) {
    ...
    const { stdout, stderr } = await $([parsedCommand])
