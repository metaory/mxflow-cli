...
global.runCommands = async(commands = []) => {
  ...
  for (const cmdName of commands) {
    ...
    // console.log('cmdName', cmdName, '<<<<<<<<<<<<<', 1)
    const cmd = raw ?? cfg.commands[cmdName]
    // console.log('cmd', cmd, '<<<<<<<<<<<<<', 2)
    const [, pickOff] = cmd.split('git_pick_')
    // console.log('PICK OFF', pickOff, '<<<<<<<<<<<<<', 3)
    pickOff && await checkoutBranch(pickOff)
    // if (pickOff) {
    //   // TODO checkout(pickOff)
    //   checkoutBranch(pickOff)
    // }
    ...
    stdout && log.green(stdout)
    stderr && log.red(stderr)
  }
}
