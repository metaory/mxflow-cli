...
import { createBranch, listLatestLogs, logBugTrackerUrl } from './git.js'
...
Object.defineProperty(global, 'cfg', {
  ...
  set: (val) => {
    ...
    L.warn('sleep' + C.yellow.dim(' is set to ') +
...
const $$ = global.$$ = async(commands = [], data = {}) => {
  for (const cmd of commands) {
    if (typeof cmd === 'object') {
      ...
      switch (key) {
        ...
        case 'checkout-branch':
          ...
          break
        case 'create-branch':
          branch = { ...branch, ...await createBranch(data, cmd[key]) }
          break
        case 'list-logs':
    ...
    const parsedCmd = pupa(cmd, branch)
