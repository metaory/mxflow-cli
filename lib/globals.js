...
global.log = ['red', 'green', 'yellow', 'blue', 'magenta', 'cyan', 'grey']
  .reduce((acc, cur) => {
    ...
    warn: (txt, etc = '') => _log(`${icons.warning} ${chalk.yellowBright(txt)} ${etc}`, 'yellow'),
    danger: (txt, etc = '') => _log(`${icons.cross} ${chalk.redBright(txt)} ${etc}`, 'red')
  })
...
global.runCommands = async(commands) => {
  for (const cmdName of commands) {
    ...
    stderr && !stdout ? spinner.fail(cmdName) : spinner.succeed(cmdName)
