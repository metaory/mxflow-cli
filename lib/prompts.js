...
export const promptBranchType = async(branchTypes) => {
  const isTrunkBased = (name) => cfg.start_workflow[name]
    .includes(`autocomplete checkout ${cfg.trunk_branch_name}`)
  const branchTypesFormat = branchTypes.map(name => { // ● ⊙
    const message = C.bold[isTrunkBased(name) ? 'blue' : 'magenta'](name)
    return { name, message }
  })
