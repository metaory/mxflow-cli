...
export async function logSleepBetweenConfig({ sleep_between_commands: sleepMilliseconds } = cfg) {
  L.warn(C.bold('sleep_between_commands') + ' is set to ' +
    `${C.bold.cyan((sleepMilliseconds / 1000).toFixed(2))} seconds`)
}

async function writeConfig(config) {
  logSleepBetweenConfig(config)
