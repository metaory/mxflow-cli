...
export async function getConfig() {
  ...
  const startWorkflowConfig = [...branchTypes, trunkBranchName].reduce((acc, cur) => {
    ...
    if (trunkBased.includes(cur)) {
      acc[cur + '__pre'] = [...new Set([
        ...acc[cur + '__pre'], `go_pick_${trunkBranchName}`
      ])]
    }
  ...
  const config = {
    ...
    bug_tracker_path: isJira
      ? `https://${jiraTenant}.atlassian.net/browse/{branchName}`
      : `https://app.clickup.com/t/${clickupTenant}/{branchName}`,
    ...defaultConfig,
