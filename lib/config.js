...
// TODO change trunk workflow commands
const startTrunkWorkflowPreTemplate = ['git_fetch_origin', 'git_pick_feature']
const startTrunkWorkflowPostTemplate = ['git_status']

const defaultConfig = {
  checkout__pre: ['git_fetch_origin'],
  commands: {
    // git_select_trunk: { from: 'trunk' },
    // git_select_feature: { from: 'feature' },
...
export async function getConfig() {
  ...
  const { trunk_based } = await multiselectInput('trunk_based', [...branchTypes])

  const workflowCommands = {
    default__pre: startWorkflowPreTemplate,
    default__post: startWorkflowPostTemplate,
    [trunk_branch_name + '__pre']: [...startTrunkWorkflowPreTemplate, `git_pick_${trunk_branch_name}`],
    [trunk_branch_name + '__post']: startTrunkWorkflowPostTemplate,
