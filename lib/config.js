...
async function writeConfig(config, sleepBetweenCommands = cfg.sleep_between_commands) {
  spinner.start(C.yellow('Saving configurations...'))
  await sleep(sleepBetweenCommands)
...
export async function getConfig() {
  ...
  const startWorkflowConfig = [trunkBranchName, ...branchTypes].reduce((acc, cur) => {
    ...
    if (trunkBased.includes(cur)) {
      acc[cur] = [
        ...acc[cur],
        `prompt_checkout_${trunkBranchName}`,
        'git_create_branch',
        'git_status'
      ]
    } else {
