...
function logYaml(str, ignoreIllegals = true) {
...
export async function getConfig() {
  ...
  const { promptSamples } = await confirmInput('promptSamples')
  let sampleConfig = {}
  if (promptSamples) {
  ...
  const config = {
    ...
    workflows: { foobar: ['echo hello word'], foobar__branch_pattern: '{branchType}/TODO_REMOVE_ME__{taskId}-{description}' },
    ...sampleConfig
  }
