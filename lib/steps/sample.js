...
export default async () => {
  const { trunkBranchName } = await stringInput("trunkBranchName", {
    value: "flight",
  });

  const branchTypes = await getBranchTypes();

  const { trunkBasedBranches } = await getTrunkBased(
    branchTypes,
    trunkBranchName
  );

  return {
    workflows: [trunkBranchName, ...branchTypes].reduce(
      (acc, cur) => ({
        ...acc,
        [`create-${cur}`]: {
          ...
          steps: [
            ...
            "git status",
            `confirm git push --set-upstream origin ${cur}/${
              cur === trunkBranchName
                ? "{description}"
                : "{taskId}-{description}"
            }`,
